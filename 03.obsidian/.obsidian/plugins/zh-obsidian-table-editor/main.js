...
var Cell = ({ row, col, content, onContentChanged, values, setValues, colJustify, setColJustify }) => {
  ...
  const showContextMenu = (event) => {
    ...
    if (row === 0) {
      menu.addItem((item) => item.setTitle("左对齐").setIcon("left-arrow").onClick(() => {
        const newColJustify = [...colJustify];
        newColJustify[col] = "LEFT";
        setColJustify(newColJustify);
      }));
      menu.addItem((item) => item.setTitle("居中对齐").setIcon("lines-of-text").onClick(() => {
        const newColJustify = [...colJustify];
        newColJustify[col] = "CENTER";
        setColJustify(newColJustify);
      }));
      menu.addItem((item) => item.setTitle("右对齐").setIcon("right-arrow").onClick(() => {
        const newColJustify = [...colJustify];
        newColJustify[col] = "RIGHT";
        setColJustify(newColJustify);
      }));
      menu.addSeparator();
      menu.addItem((item) => item.setTitle("按文本升序排序").setIcon("double-up-arrow-glyph").onClick(() => {
      ...
      menu.addItem((item) => item.setTitle("按文本降序排序").setIcon("double-down-arrow-glyph").onClick(() => {
        let newValues = [...values];
        const firstRow = newValues.shift();
      ...
      menu.addItem((item) => item.setTitle("按数字升序排序").setIcon("double-up-arrow-glyph").onClick(() => {
        let newValues = [...values];
      ...
      menu.addItem((item) => item.setTitle("按数字降序排序").setIcon("double-down-arrow-glyph").onClick(() => {
        let newValues = [...values];
        const isAllNumeric = newValues.map((row2, idx) => idx === 0 || Number.isFinite(Number.parseFloat(row2[col]))).every((r) => r === true);
        if (!isAllNumeric) {
          return;
    ...
    menu.addItem((item) => item.setTitle("在上面添加行").setIcon("up-chevron-glyph").onClick(() => {
      const colLen = values[0].length;
      const newValues = [...values];
      newValues.splice(row, 0, Array(colLen).fill(""));
      validateAndSetValues(newValues);
    }));
    menu.addItem((item) => item.setTitle("在下面添加行").setIcon("down-chevron-glyph").onClick(() => {
      const colLen = values[0].length;
      const newValues = [...values];
      newValues.splice(row + 1, 0, Array(colLen).fill(""));
      validateAndSetValues(newValues);
    }));
    menu.addItem((item) => item.setTitle("删除行").setIcon("cross").onClick(() => {
      const newValues = [...values];
      newValues.splice(row, 1);
      validateAndSetValues(newValues);
    }));
    menu.addSeparator();
    menu.addItem((item) => item.setTitle("在左侧添加列").setIcon("left-chevron-glyph").onClick(() => {
      const newValues = [...values];
    ...
    menu.addItem((item) => item.setTitle("在右侧添加列").setIcon("right-chevron-glyph").onClick(() => {
      const newValues = [...values];
    ...
    menu.addItem((item) => item.setTitle("删除列").setIcon("cross").onClick(() => {
      const newValues = [...values];
    ...
    menu.addSeparator();
    menu.addItem((item) => item.setTitle("将行上移").setIcon("up-arrow-with-tail").onClick(() => {
      if (row === 0) {
    ...
    menu.addItem((item) => item.setTitle("将行下移").setIcon("down-arrow-with-tail").onClick(() => {
    ...
    menu.addItem((item) => item.setTitle("将列右移").setIcon("right-arrow-with-tail").onClick(() => {
    ...
    menu.addItem((item) => item.setTitle("将列左移").setIcon("left-arrow-with-tail").onClick(() => {
      if (col === 0) {
...
var TableEditor = (props) => {
  ...
  const [copyText, setCopyText] = React4.useState("复制语法");
  ...
  const replaceCliked = () => {
    ...
    var 当前行号 = 当前光标.line;
    var 表文本 = view.editor.getLine(当前行号-1);
    var _reg = /^\|[^\n]+\|$/m;
    if(_reg.test(表文本)){
      var 始行;
      for(var i=当前行号-1;i>0;i--){
        var temp = view.editor.getLine(i);
        if(_reg.test(temp)){
          始行 = i;
        }else{
          始行 = i+1;
      ...
      view.editor.setSelection({line:始行,ch:0}, {line:当前行号,ch:0});
    };
  }
  ...
  return /* @__PURE__ */ React4.createElement(React4.Fragment, null, /* @__PURE__ */ React4.createElement("div", {
  ...
  }, "行数 : ", /* @__PURE__ */ React4.createElement("input", {
    type: "text",
    onChange: (e) => setNewRows(parseInt(e.target.value)),
    placeholder: "3"
  }), "列数 : ", /* @__PURE__ */ React4.createElement("input", {
    type: "text",
    onChange: (e) => setNewCols(parseInt(e.target.value)),
    placeholder: "3"
  }), /* @__PURE__ */ React4.createElement("button", {
    onClick: newTableClicked
  }, "新建表格"), /* @__PURE__ */ React4.createElement("button", {
    onClick: clearClicked
  }, "清除表格")), /* @__PURE__ */ React4.createElement("div", {
    className: "mte button-container"
    ...
    onClick: replaceCliked
  }, "更新表格")),/* @__PURE__ */ React4.createElement("button", {
    onClick: copyClicked
  }, copyText)), );
};
...
var MyPlugin = class extends import_obsidian3.Plugin {
  onload() {
    return __async(this, null, function* () {
      ...
      this.addRibbonIcon("pane-layout", "表格编辑器", () => {
        this.activateView();
      });
      this.addCommand({
  ...
  activateView() {
    return __async(this, null, function* () {
      ...
      if(data==""){
        var 当前光标 = view.editor.getCursor();
        var 当前行号 = 当前光标.line;
        var 末行行号 = view.editor.lastLine();
        var 表文本 = view.editor.getLine(当前行号);
        console.log(表文本);
        var _reg = /^\|[^\n]+\|$/m;
        if(_reg.test(表文本)){
          //data = 表文本;
          var 始行,末行,末行文本;
          for(var i=当前行号;i>0;i--){
            var temp1 = view.editor.getLine(i-1);
            if(_reg.test(temp1)){
              //data = temp1+"\n"+data;
              始行 = i-1;
            }else{
          ...
          for(var j=当前行号;j<末行行号;j++){
            var temp2 =  view.editor.getLine(j+1);
            if(_reg.test(temp2)){
              //data = data+"\n"+temp2;
              末行 = j+1;
              末行文本 = temp2;
          ...
          view.editor.setSelection({line:始行,ch:0}, {line:末行+1,ch:0});
